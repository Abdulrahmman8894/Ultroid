...
if run_as_module:
    ...
    async def safeinstall(event):
        ...
        ok = await eor(event, "`بدء تنزيل ريبثون...`")
        ...
        if plug in HELP:
            ...
            for i in HELP[plug]:
                output += i
            output += "\n© @Repthon"
            await eod(ok, f"✓ `ريبثون - تم تنزيل`: `{plug}` ✓\n\n{output}")
    ...
    def gen_chlog(repo, diff):
        ...
        ch_log = tldr_log = ""
        ch = f"<b>Repthon {ultroid_version} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b>"
        ch_tl = f"Repthon {ultroid_version} updates for {ac_br}:"
        d_form = "%d/%m/%y || %H:%M"
